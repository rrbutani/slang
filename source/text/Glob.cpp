...
SLANG_EXPORT GlobRank svGlob(const RawPath& basePath, RawPathPattern pattern_, GlobMode mode,
...
bool svGlobMatches(const RawPath& path, RawPathPattern pattern) {
    return svGlobMatchesInternal(path.asU8Str(), *pattern);
}
